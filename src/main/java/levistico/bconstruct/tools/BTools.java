...
public class BTools {
    ...
    public static void InitializeTools(String MOD_ID) {
        pickaxe = (ToolPickaxe) createTool(new ToolPickaxe(BConstruct.itemIdInc++)
                .setBaseDamageBonus(2)
        );
        hatchet = (ToolHatchet) createTool(new ToolHatchet(BConstruct.itemIdInc++)
                .setBaseDamageBonus(3)
        );
        mattock = (ToolMattock) createTool(new ToolMattock(BConstruct.itemIdInc++)
                .setBaseDamageBonus(2)
                .setDurabilityMultiplier(1.25f)
        );
        broadsword = (ToolBroadsword) createTool(new ToolBroadsword(BConstruct.itemIdInc++)
                .setBaseDamageBonus(4).setAttackDamageMultiplier(2)
                .setDurabilityMultiplier(1.1f)
        );
        kama = (ToolKama) createTool(new ToolKama(BConstruct.itemIdInc++)
                .setBaseDamageBonus(3)
        );
        pickadze = (ToolPickadze) createTool(new ToolPickadze(BConstruct.itemIdInc++)
                .setBaseDamageBonus(2)
